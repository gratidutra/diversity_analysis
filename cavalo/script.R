...
df <- df %>% mutate(`Espécie` = case_when(
  ...
  `Estação` = case_when(`Estação` == "Chuva" ~ "Rainy",
                        TRUE ~ "Dry"),
   Mês = case_when(Mês == 'Agosto' ~ 'August',
...
levels(as.factor(df$Mês))

collect_by_month <- df %>% group_by(Mês, Espécie) %>%
  summarise(total = n()) %>%
  arrange(-total) %>%  filter(total >= 12)

collect_by_month$Mês <- factor(collect_by_month$Mês,
...
df <- df %>% mutate(local_do_pouso_simplificado = case_when(`Local de pouso_1` == 'Face' |
